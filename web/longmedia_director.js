...
function defaultShot(i=0, duration=5) { return { clip_id:id("shot"), name:`Shot ${i+1}`, prompt:"", duration, seed:null, subjects:[], media_subject_id:null, cast_subject_id:null, first_frame_anchor:false, last_frame_anchor:false, source_in:0 }; }
...
function normalizeShot(s,i,ids) { s=s&&typeof s==="object"?s:{}; let seed=s.seed; seed=(seed===""||seed==null)?null:Math.max(0,Math.trunc(finite(seed,0)));
  ...
  return { clip_id:String(s.clip_id||s.id||"").trim()||id("shot"), name:String(s.name||`Shot ${i+1}`).slice(0,120), prompt:String(s.prompt||""),
    duration:clamp(s.duration,.25,150,5), seed, subjects:subs, media_subject_id:ids.has(mid)?mid:null, cast_subject_id:ids.has(cast)?cast:null,
...
function migrateV1(d) {
  ...
  const out=shots.map((s,i)=>{ const duration=clamp(s?.duration,.25,150,5); cams.push({block_id:id("camera"),start:cursor,duration,camera:{...DEFAULT_CAMERA,...(s?.camera||{})},note:""});
    ...
    cursor+=duration; return {clip_id:s?.clip_id||id("shot"),name:s?.name||`Shot ${i+1}`,prompt:s?.prompt||"",duration,seed:s?.seed??null,subjects:Array.isArray(s?.subjects)?s.subjects:[],media_subject_id:null,source_in:Math.max(0,finite(s?.source_in,0))}; });
...
function normalizeDoc(d) {
  ...
  let shots=(Array.isArray(d.shots)?d.shots:[]).slice(0,MAX_SHOTS).map((x,i)=>normalizeShot(x,i,ids)); if (!shots.length) shots=[defaultShot(0)];
...
function sceneInfoAt(doc,time){let cursor=0;for(let i=0;i<doc.shots.length;i++){const shot=doc.shots[i],end=cursor+shot.duration;if(time>=cursor&&(time<end||(i===doc.shots.length-1&&time<=end)))return {shot,index:i,start:cursor,end};cursor=end;}return null;}
...
function reflowAnchoredBlocks(doc){
  const starts=new Map();let cursor=0;
  for(const shot of doc.shots){starts.set(shot.clip_id,cursor);cursor+=shot.duration;}
...
function totalDuration(doc) { return doc.shots.reduce((n,s)=>n+Number(s.duration||0),0); }
...
function shotStart(doc,index) { let x=0; for(let i=0;i<index;i++) x+=Number(doc.shots[i]?.duration||0); return x; }
...
const BUTTON_TOOLTIPS = new Map([
  ...
  ["⟳ Preview Clip Regen","Preview a seam-safe regeneration of only the selected clip."],["🎲 Preview Reroll","Preview a new-seed regeneration of only the selected clip."],["↻ Preview From Here","Preview regeneration of the selected clip and all dependent clips after it."],
...
function attachVisualSubject(node,doc,subject,time){
  ...
  if(!hit){
    ...
    const shot=defaultShot(doc.shots.length,duration);doc.shots.push(shot);hit={shot,index:doc.shots.length-1,start:totalDuration(doc)-duration,end:totalDuration(doc)};
  }
  const shot=hit.shot;shot.media_subject_id=subject.subject_id;if(!shot.subjects.includes(subject.subject_id))shot.subjects.push(subject.subject_id);
...
function snapTime(node,doc,time,pps,{excludeIds=[],includePlayhead=false}={}){let raw=Math.max(0,finite(time,0));if(doc?.snapping===false)return raw;const fps=Math.max(1,finite(doc?.fps,24)),frame=1/fps;raw=Math.round(raw/frame)*frame;const points=timelineSnapPoints(doc,excludeIds);if(includePlayhead&&Number.isFinite(node?.__lmdPlayhead))points.push(node.__lmdPlayhead);const tol=Math.max(frame*.55,10/Math.max(28,finite(pps,54)));let best=raw,dist=Infinity;for(const p of points){const d=Math.abs(p-raw);if(d<dist){dist=d;best=p;}}return Math.round((dist<=tol?best:raw)*1e6)/1e6;}
...
function installMainTrim(block,row,node,doc,shot,index,pps){
  const left=trimHandle("left",index>0),right=trimHandle("right",true);block.append(left,right);const start0=shotStart(doc,index);
  if(index>0)left.onpointerdown=e=>{const prev=doc.shots[index-1],d0=shot.duration,p0=prev.duration,in0=finite(shot.source_in,0),media=timedMediaForShot(doc,shot),prevMax=maxBlockDuration(doc,"main",prev),boundary0=start0;beginTrim(e,node,doc,row,pps,(raw)=>{let delta=snapTime(node,doc,boundary0+raw,pps,{excludeIds:doc.shots.map(x=>x.clip_id),includePlayhead:true})-boundary0;const minDelta=Math.max(-(p0-.25),media?-in0:-Infinity),maxDelta=Math.min(d0-.25,prevMax-p0);delta=Math.max(minDelta,Math.min(maxDelta,delta));prev.duration=p0+delta;shot.duration=d0-delta;if(media)shot.source_in=in0+delta;});};
  else left.title="The first MAIN clip begins at 0s; trim its right edge or set Source In numerically.";
  right.onpointerdown=e=>{const d0=shot.duration,max=maxBlockDuration(doc,"main",shot),end0=start0+d0;beginTrim(e,node,doc,row,pps,(raw)=>{const snapped=snapTime(node,doc,end0+raw,pps,{excludeIds:doc.shots.map(x=>x.clip_id),includePlayhead:true});shot.duration=Math.max(.25,Math.min(max,snapped-start0));});};
}
function installMainReorder(block,row,node,doc,shot,index,pps){
  block.addEventListener("pointerdown",e=>{
    ...
    const x0=e.clientX,screenPerSecond=Math.max(.01,pps*pointerScale(row));let moved=false,lastX=x0;
    const move=ev=>{lastX=ev.clientX;const dx=ev.clientX-x0;if(Math.abs(dx)>5)moved=true;if(!moved)return;block.style.transform=`translateX(${dx}px)`;block.style.opacity=".78";block.style.zIndex="40";};
    const up=()=>{window.removeEventListener("pointermove",move,true);window.removeEventListener("pointerup",up,true);window.removeEventListener("pointercancel",up,true);block.style.transform="";block.style.opacity="";block.style.zIndex="";if(!moved)return;const rect=row.getBoundingClientRect(),scale=pointerScale(row),time=Math.max(0,(lastX-rect.left)/(pps*scale));let insertAt=0,cursor=0;for(let i=0;i<doc.shots.length;i++){const mid=cursor+finite(doc.shots[i].duration,0)/2;if(time>mid)insertAt=i+1;cursor+=finite(doc.shots[i].duration,0);}const oldIndex=doc.shots.indexOf(shot);if(oldIndex<0)return;doc.shots.splice(oldIndex,1);if(insertAt>oldIndex)insertAt--;insertAt=Math.max(0,Math.min(doc.shots.length,insertAt));doc.shots.splice(insertAt,0,shot);node.__lmdSelection={track:"main",id:shot.clip_id};node.__lmdSuppressMainClickUntil=performance.now()+220;commit(node,doc);};
...
async function prepareRegeneration(node,doc,shot,mode,{reroll=false}={}){const plan=await regenerationPlan(node,doc,shot,mode);plan.reroll=reroll;node.__lmdRegenPreview=plan;render(node);}
async function runPreparedRegeneration(node,doc,shot){const plan=node.__lmdRegenPreview;if(!plan||plan.clip_id!==shot.clip_id)return;if(plan.reroll)shot.seed=randomSeed();node.__lmdRegenPreview=null;await queueRegeneration(node,doc,shot,plan.mode,{reroll:false});}
async function queueRegeneration(node,doc,shot,mode,{reroll=false,extra={}}={}){
  ...
  node.__lmdLastRegen={state:"queued",request_id,clip_id:shot.clip_id,mode,seed:shot.seed??null};
  commit(node,doc);
  try{
...
function splitSelectedAtPlayhead(node,doc){
  const hit=selectedTimelineItem(node,doc);if(!hit){alert("Select a timeline clip first.");return;}const t=finite(node.__lmdPlayhead,0);if(t<=hit.start+.249||t>=hit.end-.249){alert("Place the playhead at least 0.25s inside the selected clip.");return;}const left=t-hit.start,right=hit.end-t,item=hit.item,second=JSON.parse(JSON.stringify(item));
  if(hit.kind==="main"){second.clip_id=id("shot");const base=String(item.name||directorMainTermUpper(doc)).replace(/ [AB]$/,'');item.name=`${base} A`;second.name=`${base} B`;if(timedMediaForShot(doc,item))second.source_in=finite(item.source_in,0)+left;const carriedLast=Boolean(item.last_frame_anchor);second.first_frame_anchor=false;second.last_frame_anchor=carriedLast;item.last_frame_anchor=false;item.duration=left;second.duration=right;hit.list.splice(hit.index+1,0,second);splitSceneBoundTracksForMain(doc,item.clip_id,second.clip_id,t);doc.regeneration={mode:"none",clip_id:null,request_id:null};node.__lmdSelection={track:"main",id:second.clip_id};}
  else {if(hit.kind==="camera")second.block_id=id("camera");else if(hit.kind==="audio")second.block_id=id("audio");else second.clip_id=id("layer");item.duration=left;second.start=t;second.duration=right;const media=hit.kind==="audio"?timedMediaForAudio(doc,item):hit.kind==="extra"?timedMediaForExtra(doc,item):null;if(media)second.source_in=finite(item.source_in,0)+left;if(second.scene_id){const idx=doc.shots.findIndex(s=>s.clip_id===second.scene_id);if(idx>=0)second.scene_offset=second.start-shotStart(doc,idx);}hit.list.splice(hit.index+1,0,second);node.__lmdSelection={track:hit.kind==="extra"?`extra:${hit.track.track_id}`:hit.kind,id:second[hit.idKey]};}
  commit(node,doc);
}
function addExtraTrack(node,doc,type){if((doc.extra_tracks||[]).length>=MAX_EXTRA_TRACKS)return alert(`Maximum ${MAX_EXTRA_TRACKS} extra tracks.`);const tr=defaultExtraTrack(type);doc.extra_tracks.push(tr);const hit=sceneInfoAt(doc,finite(node.__lmdPlayhead,0));const c=defaultExtraClip(tr,hit?.start??0,hit?.shot?.duration??5);tr.clips.push(c);node.__lmdSelection={track:`extra:${tr.track_id}`,id:c.clip_id};commit(node,doc);}
...
async function toolbarAction(node,doc,action){
  ...
  try {
    if(action==="split") { splitSelectedAtPlayhead(node,doc); }
    else if(action.startsWith("track_")) { addExtraTrack(node,doc,action.slice(6)); }
    else if(action==="video_prompt") { const s=defaultShot(doc.shots.length,5);if(String(doc.setup_timeline_mode)==="multiclip")s.name=`Clip ${doc.shots.length+1}`; doc.shots.push(s); setSelection(node,"main",s.clip_id); commit(node,doc); }
...
function blockBase(left,width,color,selected){return el("div",null,{position:"absolute",left:`${left}px`,top:"5px",width:`${Math.max(28,width-2)}px`,height:"60px",boxSizing:"border-box",border:`2px solid ${selected?styles.accent:"rgba(255,255,255,.13)"}`,borderRadius:"4px",background:color,color:"#fff",cursor:"pointer",overflow:"hidden",boxShadow:selected?"0 0 0 1px #0b0d10, 0 0 10px rgba(123,166,255,.35)":"none"});}
...
function renderTimeline(node,doc,host){
  const total=Math.max(.25,timelineExtent(doc)),pps=clamp(node.__lmdPps,28,120,54);node.__lmdPps=pps;if(!Number.isFinite(node.__lmdPlayhead))node.__lmdPlayhead=0;const width=Math.max(860,Math.ceil(total*pps)+40),selected=selection(node),extras=(doc.extra_tracks||[]);const frame=el("div",null,{display:"grid",gridTemplateColumns:"128px minmax(0,1fr)",border:`1px solid ${styles.border}`,borderRadius:"7px",overflow:"hidden",background:styles.bg});const labels=el("div",null,{background:"#14171c",borderRight:`1px solid ${styles.border}`});
  const addLabel=(text,color="#aab3bf",height=72,sub="",actions=[])=>{const box=el("div",null,{height:`${height}px`,boxSizing:"border-box",padding:height===24?"6px 7px":"16px 6px 0",font:"700 9px system-ui",color,borderBottom:`1px solid ${styles.border}`,whiteSpace:"nowrap",overflow:"hidden"});if(height===24){box.append(el("div",text,{font:"800 9px system-ui"}));}else{const row=el("div",null,{display:"flex",alignItems:"center",gap:"4px"});row.append(el("div",text,{font:"800 9px system-ui",overflow:"hidden",textOverflow:"ellipsis",flex:"1 1 auto"}));for(const a of actions){const b=iconButton(a.symbol,a.title,a.click,a.active,24);Object.assign(b.style,{height:"22px",fontSize:"11px",borderRadius:"4px"});row.append(b);}box.append(row);if(sub)box.append(el("div",sub,{font:"7px system-ui",opacity:".55",marginTop:"3px",overflow:"hidden",textOverflow:"ellipsis"}));}labels.append(box);};
  addLabel("TIME",styles.dim,24);addLabel(String(doc.setup_timeline_mode)==="multiclip"?"V1 · BASE / CLIPS":"V1 · BASE / MAIN","#8fc8e5",72,String(doc.setup_timeline_mode)==="multiclip"?"markers + AI takes":"cuts + AI takes",[{symbol:doc.base_audio_muted?"🔇":"🔊",title:doc.base_audio_muted?"Unmute embedded audio from BASE video":"Mute embedded audio from BASE video",active:doc.base_audio_muted,click:()=>{doc.base_audio_muted=!doc.base_audio_muted;commit(node,doc);}}]);addLabel("C1 · CAMERA","#c1a1e8");addLabel("A1 · AUDIO","#8bd5a5",72,"monitor mix",[{symbol:doc.audio_track_muted?"🔇":"🔊",title:doc.audio_track_muted?"Unmute A1 monitor playback":"Mute A1 monitor playback",active:doc.audio_track_muted,click:()=>{doc.audio_track_muted=!doc.audio_track_muted;commit(node,doc);}}]);for(const tr of extras){const actions=[];if(["audio","video","reference","character"].includes(tr.type))actions.push({symbol:tr.muted?"🔇":"🔊",title:tr.muted?"Unmute this layer for monitor playback":"Mute this layer for monitor playback",active:tr.muted,click:()=>{tr.muted=!tr.muted;commit(node,doc);}});addLabel(`${tr.enabled===false?"○":"●"} ${tr.name}`,EXTRA_TRACK_META[tr.type]?.color||"#bbb",72,EXTRA_TRACK_META[tr.type]?.label||tr.type,actions);}
  const scroll=el("div",null,{overflowX:"auto",overflowY:"auto",scrollBehavior:"auto",maxHeight:`${Math.min(560,24+72*(3+extras.length))}px`});scroll.scrollLeft=finite(node.__lmdScrollLeft,0);scroll.onscroll=()=>{node.__lmdScrollLeft=scroll.scrollLeft;};const inner=el("div",null,{width:`${width}px`}),timeRuler=ruler(total,pps,width);inner.append(timeRuler);const main=trackRow(width),cam=trackRow(width),aud=trackRow(width),rows=[main,cam,aud];let cursor=0;
  doc.shots.forEach((s,i)=>{
    const planned=(node.__lmdRegenPreview?.indices||[]).includes(i),b=blockBase(cursor*pps,s.duration*pps,styles.main,selected.track==="main"&&selected.id===s.clip_id);
    if(planned){b.style.outline="3px solid #ffb24a";b.style.outlineOffset="-3px";}
    const ts=node.__lmdTakeStatus?.clips?.[s.clip_id];if(ts?.take_count)b.append(el("div",`${ts.take_count} take${ts.take_count===1?"":"s"}`,{position:"absolute",right:"16px",bottom:"4px",zIndex:"6",background:"rgba(0,0,0,.72)",border:"1px solid rgba(255,255,255,.25)",borderRadius:"3px",padding:"2px 4px",font:"8px system-ui",pointerEvents:"none"}));
    const mediaSub=subjectById(doc,s.media_subject_id);if(mediaSub?.media)decorateMedia(b,mediaSub.media,{sourceIn:s.source_in||0,sourceDuration:s.duration});
    const src=mediaSourceCaption(mediaSub?.media,s.source_in||0,s.duration),pending=doc.regeneration?.clip_id===s.clip_id?(doc.regeneration.mode==="from_here"?" ↻":" ⟳"):"";const anchorBadges=`${s.first_frame_anchor?" · FIRST":""}${s.last_frame_anchor?" · LAST":""}`;
    blockLabel(b,`${displayMainName(doc,s,i)}${pending}${anchorBadges}`,`${cursor.toFixed(2)}–${(cursor+s.duration).toFixed(2)}s${src?` · ${src}`:""}`);
    b.onclick=e=>{e.stopPropagation();if(performance.now()<finite(node.__lmdSuppressMainClickUntil,0))return;setSelection(node,"main",s.clip_id);};
    installMainTrim(b,main,node,doc,s,i,pps);installMainReorder(b,main,node,doc,s,i,pps);main.append(b);cursor+=s.duration;
  });
  if(String(doc.setup_timeline_mode)==="multiclip"&&doc.shots.length>1){
    const selectedBoundary=selectedMulticlipBoundaryIndex(node,doc);
    for(let i=1;i<doc.shots.length;i++){const x=shotStart(doc,i)*pps,active=i===selectedBoundary,marker=el("div",null,{position:"absolute",left:`${x-6}px`,top:"-2px",width:"12px",height:"66px",zIndex:"35",cursor:"ew-resize",pointerEvents:"auto"});marker.title="MultiClip boundary · drag to change the adjacent clip durations";marker.append(el("div",null,{position:"absolute",left:"5px",top:"0",bottom:"0",width:active?"2px":"1px",background:active?"#ffcf67":"#f1f4f8",boxShadow:active?"0 0 5px rgba(255,207,103,.85)":"0 0 3px rgba(0,0,0,.8)"}),el("div",null,{position:"absolute",left:"2px",top:"0",width:"0",height:"0",borderLeft:"4px solid transparent",borderRight:"4px solid transparent",borderTop:`7px solid ${active?"#ffcf67":"#f1f4f8"}`}));marker.onclick=e=>{e.preventDefault();e.stopPropagation();setMulticlipBoundarySelection(node,doc,i);render(node);};installMulticlipBoundary(marker,main,node,doc,i,pps);main.append(marker);}
  }
  doc.camera_blocks.forEach(c=>{const b=blockBase(c.start*pps,c.duration*pps,styles.camera,selected.track==="camera"&&selected.id===c.block_id);blockLabel(b,c.camera?.movement||"Camera",`${c.start.toFixed(2)}–${(c.start+c.duration).toFixed(2)}s`);b.onclick=e=>{e.stopPropagation();setSelection(node,"camera",c.block_id);};installFreeTrim(b,cam,node,doc,c,"camera",pps);installFreeMove(b,cam,node,doc,c,"camera",pps);cam.append(b);});
  doc.audio_blocks.forEach(a=>{const b=blockBase(a.start*pps,a.duration*pps,styles.audio,selected.track==="audio"&&selected.id===a.block_id),sub=subjectById(doc,a.subject_id);if(sub?.media)decorateMedia(b,sub.media,{audioVisual:true,sourceIn:a.source_in||0,sourceDuration:a.duration});blockLabel(b,a.kind==="reference"?(sub?.name||"Audio reference"):(a.prompt||"Sound prompt"),`${a.role} · ${a.start.toFixed(2)}–${(a.start+a.duration).toFixed(2)}s`);b.onclick=e=>{e.stopPropagation();setSelection(node,"audio",a.block_id);};installFreeTrim(b,aud,node,doc,a,"audio",pps);installFreeMove(b,aud,node,doc,a,"audio",pps);aud.append(b);});inner.append(main,cam,aud);
  for(const tr of extras){const row=trackRow(width);rows.push(row);row.style.opacity=tr.enabled===false?".42":"1";for(const c of tr.clips||[]){const meta=EXTRA_TRACK_META[tr.type]||{},b=blockBase(c.start*pps,c.duration*pps,meta.color||"#555",selected.track===`extra:${tr.track_id}`&&selected.id===c.clip_id),sub=subjectById(doc,c.subject_id);if(sub?.media)decorateMedia(b,sub.media,{audioVisual:tr.type==="audio",sourceIn:c.source_in||0,sourceDuration:c.duration});blockLabel(b,tr.type==="embedding"?(c.embedding_name||c.name||meta.label):(c.name||meta.label),`${c.start.toFixed(2)}–${(c.start+c.duration).toFixed(2)}s${tr.type==="embedding"&&c.embedding_name?` · ${c.embedding_name}`:c.prompt?" · prompt":""}`);b.onclick=e=>{e.stopPropagation();setSelection(node,`extra:${tr.track_id}`,c.clip_id);};if(!tr.locked){installFreeTrim(b,row,node,doc,c,"extra",pps);installFreeMove(b,row,node,doc,c,"extra",pps,`extra:${tr.track_id}`);}row.append(b);}row.addEventListener("dblclick",e=>{if(tr.locked)return;const rect=row.getBoundingClientRect(),scale=pointerScale(row),time=snapTime(node,doc,Math.max(0,(e.clientX-rect.left)/(pps*scale)),pps,{includePlayhead:true}),c=defaultExtraClip(tr,time,5);tr.clips.push(c);node.__lmdSelection={track:`extra:${tr.track_id}`,id:c.clip_id};commit(node,doc);});inner.append(row);}
  const timeFromClient=clientX=>{const rect=main.getBoundingClientRect(),scale=pointerScale(main);return Math.max(0,Math.min(total,(clientX-rect.left)/(pps*scale)));},setFromEvent=e=>{if(e.defaultPrevented)return;stopPlayback(node);setPlayheadTime(node,doc,snapTime(node,doc,timeFromClient(e.clientX),pps));};for(const row of rows)row.addEventListener("click",setFromEvent);
...
async function moveTakeToFolder(node,doc,shot,take,folder){try{const r=await api.fetchApi('/longmedia/director/move_take',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({project_id:doc.project_id,clip_id:shot.clip_id,revision:String(take.revision||""),folder:String(folder||"Unsorted")})});const data=await r.json();if(!r.ok)throw new Error(data.error||`HTTP ${r.status}`);node.__lmdTakeFolderCurrent=String(folder||"Unsorted");await refreshTakeStatus(node,node.__lmdDoc||doc);}catch(err){alert(`Move TAKE failed: ${err?.message||err}`);}}
async function deleteTake(node,doc,shot){const status=node.__lmdTakeStatus?.clips?.[shot.clip_id],revision=selectedTakeRevision(node,shot,status);if(!revision)return alert("No take selected to delete.");const take=(status?.takes||[]).find(t=>String(t.revision||"")===revision);const label=`${take?.active?"ACTIVE ":""}${take?.draft?"EMPTY ":""}take ${revision.slice(0,8)}`;if(!window.confirm(`Delete ${label}?\n\nThis removes the TAKE entry from Director. Rendered output media files are not deleted. This action cannot be undone.`))return;try{const r=await api.fetchApi('/longmedia/director/delete_take',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({project_id:doc.project_id,clip_id:shot.clip_id,revision})});const data=await r.json();if(!r.ok)throw new Error(data.error||`HTTP ${r.status}`);if(node.__lmdSelectedTakeByClip)delete node.__lmdSelectedTakeByClip[shot.clip_id];clearLiveLatentPreview(node);await refreshTakeStatus(node,node.__lmdDoc||doc);const liveDoc=node.__lmdDoc||doc,liveShot=(liveDoc.shots||[]).find(s=>s.clip_id===shot.clip_id)||liveDoc.shots?.[0],nextStatus=node.__lmdTakeStatus?.clips?.[shot.clip_id],fallbackRevision=String(nextStatus?.editor_active_revision||nextStatus?.active_revision||nextStatus?.active?.revision||nextStatus?.takes?.[0]?.revision||"");if(fallbackRevision&&liveShot){node.__lmdSelectedTakeByClip=node.__lmdSelectedTakeByClip||{};node.__lmdSelectedTakeByClip[shot.clip_id]=fallbackRevision;const fallback=(nextStatus?.takes||[]).find(t=>String(t.revision||"")===fallbackRevision);if(fallback?.director_timeline_snapshot)applyTakeTimelineSnapshot(node,liveDoc,fallback);else render(node);}else{const emptyDoc=buildEmptyTakeWorkingDoc(liveDoc,shot.clip_id),gp=widget(node,"global_prompt");if(gp){gp.value="";try{gp.callback?.(gp.value);}catch(_){}}invalidateDirectorMediaViews(node);commit(node,emptyDoc);}await refreshTakeStatus(node,node.__lmdDoc||liveDoc);}catch(err){alert(`Delete take failed: ${err?.message||err}`);}}
function renderShotInspector(node,doc,shot,host){const index=doc.shots.indexOf(shot),start=shotStart(doc,index),media=timedMediaForShot(doc,shot),mainTerm=directorMainTermUpper(doc),mainName=displayMainName(doc,shot,index);host.append(el("div",`MAIN · ${mainTerm} ${index+1} · ${mainName}`,{font:"700 11px system-ui",color:"#9bd4f1",marginBottom:"6px"}));
  ...
  else regenInfo.textContent="Selective regeneration stores native AV takes per clip. Clip Only keeps both seams compatible; From Here is the safe fallback after structural timeline changes.";
  host.append(regenInfo);if(node.__lmdRegenPreview?.clip_id===shot.clip_id){const p=node.__lmdRegenPreview;regenInfo.textContent=`PREVIEW · will render clips ${p.indices.map(i=>i+1).join(", ")} · ${p.reason}`;const run=iconButton("▶",p.reroll?"Run the prepared reroll":"Run the prepared regeneration",()=>runPreparedRegeneration(node,doc,shot),true,38);run.style.marginTop="6px";host.append(run);}
}
...
function buildEmptyTakeWorkingDoc(sourceDoc, selectedClipId=""){
  ...
  out.camera_blocks=(out.shots||[]).map((shot,i)=>{
    const block=defaultCameraBlock(cursor,shot.duration,shot.clip_id,0);
...
function renderTakeSidebar(node,doc,host){
  const shot=takeShotForSidebar(node,doc);host.replaceChildren();
  if(!shot){host.append(el("div","NO MAIN SHOT",{font:"800 9px system-ui",color:styles.dim}));return;}
  const status=node.__lmdTakeStatus?.clips?.[shot.clip_id]||{},takes=status.takes||[],selectedRevision=selectedTakeRevision(node,shot,status);
  const folders=(node.__lmdTakeStatus?.folders||[{name:"Unsorted",default:true}]).map(f=>String(f.name||"")).filter(Boolean);
  if(!folders.includes("Unsorted"))folders.unshift("Unsorted");
  if(!folders.includes(String(node.__lmdTakeFolderCurrent||"")))node.__lmdTakeFolderCurrent="Unsorted";
  const head=el("div",null,{position:"sticky",top:"0",zIndex:"5",background:"linear-gradient(180deg,#171b21 88%,rgba(23,27,33,0))",paddingBottom:"8px"});
  const titleRow=el("div",null,{display:"flex",alignItems:"center",justifyContent:"space-between",gap:"6px"});
  titleRow.append(el("div","TAKES",{font:"900 11px system-ui",letterSpacing:".08em",color:"#c7d0dc"}),iconButton("+F","Create project folder inside longmedia_director",()=>createTakeFolder(node,doc),false,30));head.append(titleRow);
  head.append(el("div",`${displayMainName(doc,shot,doc.shots.indexOf(shot))} · ${takes.length}`,{font:"8px system-ui",color:styles.dim,marginTop:"2px"}));
  const manage=el("div",null,{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(88px,1fr))",gap:"5px",marginTop:"7px"});
  manage.append(button("CREATE",()=>createTake(node,doc,shot)),button("DUPLICATE",()=>duplicateTake(node,doc,shot),Boolean(selectedRevision)),button("DELETE",()=>deleteTake(node,doc,shot),Boolean(selectedRevision)),button("REFRESH",()=>refreshTakeStatus(node,doc)));head.append(manage);
  const tabs=el("div",null,{display:"flex",gap:"4px",marginTop:"7px",overflowX:"auto",paddingBottom:"2px"});
  for(const folder of folders){const b=button(folder,()=>{node.__lmdTakeFolderCurrent=folder;render(node);},String(node.__lmdTakeFolderCurrent)===folder,`TAKE project folder: ${folder}. Drop a TAKE card here to move it.`);b.ondragover=e=>{e.preventDefault();if(e.dataTransfer)e.dataTransfer.dropEffect="move";};b.ondrop=e=>{e.preventDefault();const rev=e.dataTransfer?.getData("application/x-longmedia-take");const take=takes.find(t=>String(t.revision||"")===String(rev||""));if(take)moveTakeToFolder(node,doc,shot,take,folder);};tabs.append(b);}head.append(tabs);host.append(head);
  const current=String(node.__lmdTakeFolderCurrent||"Unsorted"),visible=takes.filter(t=>String(t.library_folder||"Unsorted")===current);
  if(!visible.length){host.append(el("div",`No takes in ${current}. CREATE adds an active empty TAKE here.`,{font:"9px/1.4 system-ui",color:styles.dim,padding:"10px 2px"}));return;}
  ...
  for(const t of visible){const revision=String(t.revision||""),selected=revision===selectedRevision,draft=Boolean(t.draft)||String(t.state||"")==="draft",name=String(t.take_name||t.label||`Take ${revision.slice(0,8)}`);const card=el("div",null,{background:"#101319",border:`1px solid ${selected?"#ffcf67":styles.border}`,borderRadius:"7px",padding:"6px",boxShadow:selected?"0 0 0 1px rgba(255,207,103,.32)":"none",cursor:"pointer",minWidth:"0"});card.draggable=true;card.ondragstart=e=>{e.stopPropagation();e.dataTransfer?.setData("application/x-longmedia-take",revision);if(e.dataTransfer)e.dataTransfer.effectAllowed="move";};card.onclick=()=>restoreTake(node,node.__lmdDoc||doc,shot,revision,{applyTimeline:true,takeHint:t});
    ...
    const nameRow=el("div",null,{display:"flex",alignItems:"center",gap:"4px",marginTop:"6px"});nameRow.append(el("div",`${selected?"● ":""}${name}`,{font:"800 10px system-ui",color:selected?"#fff0bd":"#e5e9ef",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",flex:"1 1 auto"}));const rn=iconButton("✎","Rename TAKE and its folder",()=>renameTake(node,doc,shot,t),false,28);rn.onclick=e=>{e.stopPropagation();renameTake(node,node.__lmdDoc||doc,shot,t);};nameRow.append(rn);card.append(nameRow);
    card.append(el("div",new Date((t.created_at_unix||0)*1000).toLocaleString(),{font:"8px system-ui",color:"#aab3bf",marginTop:"1px",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}));
    const state=draft?"empty":t.render_active?"render active":"cached";card.append(el("div",`${state} · seed ${t.effective_seed??"—"}`,{font:"7px system-ui",color:styles.dim,marginTop:"3px",textTransform:"uppercase",letterSpacing:".04em"}));
    const actions=el("div",null,{display:"flex",gap:"4px",marginTop:"5px"});const restore=iconButton("↩",draft?"Restore this empty TAKE settings snapshot":"Restore this cached take and its authored Director timeline metadata",()=>restoreTake(node,doc,shot,t.revision),false,30);restore.onclick=e=>{e.stopPropagation();restoreTake(node,node.__lmdDoc||doc,shot,t.revision,{applyTimeline:true,takeHint:t});};actions.append(restore);if(!draft){const recast=iconButton("🎭","Recast from this rendered take",()=>recastTake(node,doc,shot,t),true,30);recast.onclick=e=>{e.stopPropagation();recastTake(node,doc,shot,t);};actions.append(recast);}card.append(actions);list.append(card);
  }
  host.append(list);
}
...
function renderProgramMonitor(node,doc,host){
  ...
  const meta=el("div",null,{height:"24px",display:"flex",alignItems:"center",justifyContent:"center",padding:"0 6px",boxSizing:"border-box"});const detail=el("div","",{font:"9px/1.3 system-ui",color:styles.dim,textAlign:"center",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",maxWidth:"90%"});node.__lmdPreviewDetail=detail;meta.append(detail);card.append(meta);renderTransport(node,doc,card);host.append(card);
...
function renderInspector(node,doc,host){const s=selection(node);const panel=el("div",null,{background:styles.panel,border:`1px solid ${styles.border}`,borderRadius:"5px",padding:"8px",minHeight:"170px"});if(s.track==="main"){const b=doc.shots.find(x=>x.clip_id===s.id)||doc.shots[0];if(b){node.__lmdSelection={track:"main",id:b.clip_id};renderShotInspector(node,doc,b,panel);}}else if(s.track==="camera"){const b=doc.camera_blocks.find(x=>x.block_id===s.id);if(b)renderCameraInspector(node,doc,b,panel);}else if(s.track==="audio"){const b=doc.audio_blocks.find(x=>x.block_id===s.id);if(b)renderAudioInspector(node,doc,b,panel);}else if(String(s.track||"").startsWith("extra:")){const tid=String(s.track).slice(6),tr=(doc.extra_tracks||[]).find(t=>t.track_id===tid),c=tr?.clips?.find(x=>x.clip_id===s.id);if(tr&&c)renderExtraInspector(node,doc,tr,c,panel);}host.append(panel);}
...
function render(node){if(!isDirector(node))return;loadDirectorUIState(node);for(const ro of node.__lmdResizeObservers||[])try{ro.disconnect();}catch(_){}node.__lmdResizeObservers=[];const editor=ensureEditor(node);const root=editor?.element||node.__lmdRoot;if(!root)return;const doc=node.__lmdDoc??parseDoc(node);node.__lmdDoc=doc;
  ...
  if(node.__lmdShowTakes===undefined)node.__lmdShowTakes=true;toolbar.append(button("TAKES",()=>{node.__lmdShowTakes=!node.__lmdShowTakes;saveDirectorUIPref(node,"show_takes",Boolean(node.__lmdShowTakes));render(node);},Boolean(node.__lmdShowTakes),"Show / hide the full-height TAKE gallery"),iconButton("↶","Undo Director edit (Ctrl/Cmd+Z)",()=>undo(node)),iconButton("↷","Redo Director edit (Ctrl/Cmd+Y)",()=>redo(node)),iconButton("⇆",doc.ripple_tracks!==false?"Ripple ON · CAMERA/AUDIO follow their MAIN scene when clips are reordered or durations change":"Ripple OFF · CAMERA/AUDIO keep absolute timeline positions",()=>{doc.ripple_tracks=doc.ripple_tracks===false;commit(node,doc);},doc.ripple_tracks!==false),iconButton("🧲","Toggle Magnet snapping for playhead, trims and movable clips",()=>{doc.snapping=doc.snapping===false;commit(node,doc);},doc.snapping!==false));if(String(doc.setup_timeline_mode)==="multiclip"){toolbar.append(button("+ CLIP",()=>addMulticlipAtPlayhead(node,doc),false,"Add MultiClip marker at the playhead and split the current clip"),button("− CLIP",()=>deleteMulticlipBoundary(node,doc),selectedMulticlipBoundaryIndex(node,doc)>0,"Delete the selected/nearest MultiClip marker and merge adjacent clips"));}
  const divider=()=>el("span","",{width:"1px",height:"20px",background:styles.border,margin:"0 2px",flex:"0 0 auto"});toolbar.append(divider());
  for(const [symbol,action,title] of [["✂","split","Split only the selected timeline clip at the playhead (B)"],["▣","video_prompt",String(doc.setup_timeline_mode)==="multiclip"?"Add MAIN clip":"Add MAIN shot"],["◉","camera_prompt","Add camera block"],["♪","sound_prompt","Add sound prompt"],["T","track_prompt","Add prompt layer"],["✦","track_embedding","Add MiniMax H3 embedding layer"],["♙","track_character","Add character layer"],["◆","track_reference","Add reference layer"],["▤","track_video","Add video layer"],["♫","track_audio","Add audio layer"],["▧","image","Import image"],["♬","audio","Import audio"],["▶","video","Import video"]])toolbar.append(iconButton(symbol,title,()=>toolbarAction(node,doc,action)));
  toolbar.append(divider(),el("span","⌕",{font:"12px system-ui",color:styles.dim,marginLeft:"2px"}));const z=input(node.__lmdPps||54,v=>{node.__lmdPps=clamp(v,28,120,54);saveDirectorUIPref(node,"timeline_zoom",node.__lmdPps);render(node);},"range");z.min="28";z.max="120";z.step="2";z.style.width="72px";z.oninput=()=>{node.__lmdPps=clamp(z.value,28,120,54);saveDirectorUIPref(node,"timeline_zoom",node.__lmdPps);};z.onchange=()=>render(node);toolbar.append(z);root.append(toolbar);
  const showTakes=Boolean(node.__lmdShowTakes);const takesWidth=clamp(node.__lmdTakesWidth,190,900,260);node.__lmdTakesWidth=takesWidth;const body=el("div",null,{display:"grid",gridTemplateColumns:showTakes?`${takesWidth}px minmax(0,1fr) 318px`:"minmax(0,1fr) 318px",gap:"9px",height:"calc(100% - 78px)",minHeight:"0",boxSizing:"border-box",padding:"9px",background:"linear-gradient(180deg,#111318 0%,#0f1115 100%)"});if(showTakes){const takesSide=el("aside",null,{position:"relative",background:"linear-gradient(180deg,#171b21,#13161b)",border:`1px solid ${styles.border}`,borderRadius:"7px",padding:"8px 12px",overflowY:"auto",overflowX:"hidden",direction:"rtl",scrollbarGutter:"stable",minHeight:"0",minWidth:"190px",boxShadow:"0 9px 24px rgba(0,0,0,.18)"});const grip=el("div",null,{position:"absolute",top:"0",right:"0",width:"9px",height:"100%",cursor:"ew-resize",zIndex:"20",background:"linear-gradient(90deg,transparent,rgba(123,166,255,.10))"});grip.append(el("div",null,{position:"absolute",top:"50%",right:"2px",width:"3px",height:"42px",transform:"translateY(-50%)",borderRadius:"3px",background:"#566171"}));attachDirectorTooltip(grip,"Drag horizontally to resize the TAKE gallery. Wider galleries automatically use multiple columns.");grip.onpointerdown=e=>{e.preventDefault();e.stopPropagation();const startX=e.clientX,startW=node.__lmdTakesWidth,startNodeW=Math.max(NODE_MIN_WIDTH,Number(node.size?.[0])||NODE_DEFAULT_WIDTH);const move=ev=>{const w=clamp(startW+(ev.clientX-startX),190,900,startW),delta=w-startW;node.__lmdTakesWidth=w;body.style.gridTemplateColumns=`${w}px minmax(0,1fr) 318px`;const nodeW=Math.max(NODE_MIN_WIDTH,startNodeW+delta);try{node.setSize?.([nodeW,Number(node.size?.[1])||NODE_HEIGHT]);}catch(_){if(node.size)node.size[0]=nodeW;}node.setDirtyCanvas?.(true,true);app.canvas?.setDirty?.(true,true);};const up=()=>{saveDirectorUIPref(node,"takes_width",Math.round(node.__lmdTakesWidth));node.setDirtyCanvas?.(true,true);window.removeEventListener("pointermove",move,true);window.removeEventListener("pointerup",up,true);window.removeEventListener("pointercancel",up,true);};window.addEventListener("pointermove",move,true);window.addEventListener("pointerup",up,true);window.addEventListener("pointercancel",up,true);};takesSide.append(grip);const takesContent=el("div",null,{minWidth:"0",width:"100%",direction:"ltr"});renderTakeSidebar(node,doc,takesContent);takesSide.append(takesContent);body.append(takesSide);}
  const left=el("div",null,{display:"grid",gridTemplateRows:`${PROGRAM_MONITOR_HEIGHT}px auto auto 126px`,gap:"9px",minWidth:"0",minHeight:"0",overflowY:"auto",paddingRight:"2px"});
  const monitorBox=el("div",null,{minWidth:"0"});renderProgramMonitor(node,doc,monitorBox);left.append(monitorBox);
  const dynamicTimelineHeight=Math.min(590,TIMELINE_PANEL_HEIGHT+72*(doc.extra_tracks?.length||0));const timelineBox=el("div",null,{overflow:"hidden",minHeight:`${dynamicTimelineHeight}px`,background:styles.panel,borderRadius:"7px",boxShadow:"0 7px 20px rgba(0,0,0,.15)"});renderTimeline(node,doc,timelineBox);left.append(timelineBox);
  const inspectorHeight=clamp(node.__lmdInspectorHeight,260,900,420);node.__lmdInspectorHeight=inspectorHeight;const inspectorWrap=el("div",null,{position:"relative",height:`${inspectorHeight}px`,minHeight:"260px",maxHeight:"900px",minWidth:"0",boxSizing:"border-box"});const inspector=el("div",null,{height:"100%",overflowY:"auto",minHeight:"0",background:"linear-gradient(180deg,#15191f,#12151a)",border:`1px solid ${styles.border}`,borderRadius:"7px",padding:"8px 8px 13px",boxSizing:"border-box",boxShadow:"0 7px 20px rgba(0,0,0,.15)"});node.__lmdInspectorHost=inspector;inspector.scrollTop=finite(node.__lmdInspectorScrollTop,0);inspector.onscroll=()=>{node.__lmdInspectorScrollTop=inspector.scrollTop;};renderInspector(node,doc,inspector);requestAnimationFrame(()=>{if(inspector.isConnected)inspector.scrollTop=finite(node.__lmdInspectorScrollTop,0);});const inspectorGrip=el("div",null,{position:"absolute",left:"0",right:"0",bottom:"0",height:"10px",cursor:"ns-resize",zIndex:"25",borderRadius:"0 0 7px 7px",background:"linear-gradient(180deg,transparent,rgba(123,166,255,.13))"});inspectorGrip.append(el("div",null,{position:"absolute",left:"50%",bottom:"2px",width:"42px",height:"3px",transform:"translateX(-50%)",borderRadius:"3px",background:"#566171"}));attachDirectorTooltip(inspectorGrip,"Drag to resize the Shot / Clip inspector. This size is saved with the Director node.");inspectorGrip.onpointerdown=e=>{e.preventDefault();e.stopPropagation();const y0=e.clientY,h0=inspectorWrap.getBoundingClientRect().height;const move=ev=>{const h=clamp(h0+(ev.clientY-y0),260,900,h0);node.__lmdInspectorHeight=h;inspectorWrap.style.height=`${h}px`;};const up=()=>{saveDirectorUIPref(node,"inspector_height",Math.round(node.__lmdInspectorHeight));window.removeEventListener("pointermove",move,true);window.removeEventListener("pointerup",up,true);window.removeEventListener("pointercancel",up,true);};window.addEventListener("pointermove",move,true);window.addEventListener("pointerup",up,true);window.addEventListener("pointercancel",up,true);};inspectorWrap.append(inspector,inspectorGrip);left.append(inspectorWrap);
  const global=el("div",null,{background:"linear-gradient(180deg,#171b21,#13161b)",border:`1px solid ${styles.border}`,borderRadius:"7px",padding:"8px",overflow:"hidden",boxShadow:"0 7px 20px rgba(0,0,0,.14)"});const gp=widget(node,"global_prompt");global.append(el("div","GLOBAL PROMPT",{font:"800 9px system-ui",letterSpacing:".08em",color:"#c0c8d3",marginBottom:"5px"}),persistentTextarea(node,"global:prompt",gp?.value||"",v=>{if(gp){gp.value=v;try{gp.callback?.(v);}catch(_){}}},3));left.append(global);
...
function ensureEditor(node){if(node.__lmdEditor)return node.__lmdEditor;if(typeof node.addDOMWidget!=="function")return null;hideWidget(widget(node,"director_json"));hideWidget(widget(node,"global_prompt"));
  ...
  const root=el("div",null,{width:"100%",height:"100%",minHeight:`${EDITOR_MIN_HEIGHT}px`,boxSizing:"border-box",background:styles.bg,color:styles.text,font:"11px system-ui",overflow:"hidden",border:`1px solid ${styles.border}`,borderRadius:"6px"});root.tabIndex=0;root.addEventListener("keydown",e=>{const tag=String(e.target?.tagName||"").toLowerCase(),typing=["input","textarea","select"].includes(tag);if((e.ctrlKey||e.metaKey)&&String(e.key).toLowerCase()==="z"){e.preventDefault();e.shiftKey?redo(node):undo(node);}else if((e.ctrlKey||e.metaKey)&&String(e.key).toLowerCase()==="y"){e.preventDefault();redo(node);}else if(!typing&&e.code==="Space"){e.preventDefault();startPlayback(node,node.__lmdDoc??parseDoc(node));}else if(!typing&&e.key==="ArrowLeft"){e.preventDefault();stopPlayback(node);const d=node.__lmdDoc??parseDoc(node);setPlayheadTime(node,d,finite(node.__lmdPlayhead,0)-1);}else if(!typing&&e.key==="ArrowRight"){e.preventDefault();stopPlayback(node);const d=node.__lmdDoc??parseDoc(node);setPlayheadTime(node,d,finite(node.__lmdPlayhead,0)+1);}else if(!typing&&e.key==="Home"){e.preventDefault();stopPlayback(node);setPlayheadTime(node,node.__lmdDoc??parseDoc(node),0);}else if(!typing&&e.key==="End"){e.preventDefault();stopPlayback(node);const d=node.__lmdDoc??parseDoc(node);setPlayheadTime(node,d,totalDuration(d));}else if(!typing&&String(e.key).toLowerCase()==="b"){e.preventDefault();splitSelectedAtPlayhead(node,node.__lmdDoc??parseDoc(node));}});
