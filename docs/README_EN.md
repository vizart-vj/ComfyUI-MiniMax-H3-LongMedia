# ComfyUI-MiniMax-H3-LongMedia Documentation — English

This release ships only current user-facing documentation. Every topic has an English (`*_EN.md`) and Russian (`*_RU.md`) version.

## Start here

- [LongMedia Director — Complete Guide](DIRECTOR_GUIDE_EN.md)
- [Operating Modes](MODES_GUIDE_EN.md)
- [Audio Modes and `video_ref_edit`](AUDIO_MODES_GUIDE_EN.md)
- [Integrated Refine and Latent Hi-Res](TWO_PASS_LATENT_HIRES_REFINER_GUIDE_EN.md)
- [Sampler, VRAM and Performance](SAMPLER_OPTIMIZATION_EN.md)
- [Architecture](ARCHITECTURE_EN.md)

## Director

- [Director Complete Guide](DIRECTOR_GUIDE_EN.md)
- [Director Selective Regeneration and TAKE workflow](DIRECTOR_REGENERATION_GUIDE_EN.md)
- [Cameras Guide](CAMERAS_GUIDE_EN.md)

## MultiClip and prompting

- [MultiClip Quick Guide](MULTICLIP_GUIDE_EN.md)
- [MultiClip Prompting Guide](MULTICLIP_PROMPTING_GUIDE_EN.md)
- [Fixed Segmentation Prompting Rules](PROMPTING_SEGMENTATION_EN.md)
- [System Prompt — MultiClip + Cameras](SYSTEM_PROMPT_MULTICLIP_CAMERAS_EN.md)

## Release

- [0.6.54 Release Notes](RELEASE_NOTES_0.6.54_EN.md)
- [0.6.40 Release Notes](RELEASE_NOTES_0.6.40_EN.md)

Historical development notes are consolidated in the root [CHANGELOG](../CHANGELOG.md) instead of being shipped as dozens of separate release-note files.
